 ...
 */
@Component
public class CouponGetJudgeNodeHandler extends BaseJudgeNodeHandler {
    ...
    public List<TargetUserDTO> operate(DelegateExecution delegateExecution, String innerParam) throws BusiException {
        try {
            ...
            if (StringUtils.isEmpty(couponCode)) {
                logger.warn("CouponGetJudgeFlowNode.operate() error! couponCode(selectedId) is empty!!");
                return null;
            }
            // 根据优惠券id 获取领取成功的客群
            CouponService couponService = SpringContextUtils.getBean(CouponService.class);
            List<TargetUserDTO> targetUserListAll = couponService.listReceiveMember(couponCode).getData()
            ...
            // List<Long> targetUserListAll = Lists.newArrayList(1L, 2L, 3L, 4L);
            logger.info("CouponGetJudgeFlowNode.operate(),targetUserListAll={}", targetUserListAll);
            // logger.info("param=:", param.);
            return targetUserListAll;
        }
        catch (Exception e) {
            logger.error("CouponGetJudgeFlowNode.operate() error! msg={}", e.getMessage());
            return null;
        }
    ...
    public String getNodeType() {
        return MarketingNodeExecuteTypeEnum.COUPON_GET.getCode();
    }
